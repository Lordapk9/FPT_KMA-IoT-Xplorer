...
class Face_Register:
    def __init__(self):
        ...
        self.path_photos_from_camera = "D:/Project/2024/FPT_SmartHome/face_recognition/Data/"
    ...
    #     if os.listdir("D:/Project/2024/FPT_SmartHome/face_recognition/Data/"):
    ...
    def check_existing_faces_cnt(self):
        ...
        if os.listdir("D:/Project/2024/FPT_SmartHome/face_recognition/Data/"):
            # Lấy danh sách các thư mục hoặc tệp
            person_list = os.listdir("D:/Project/2024/FPT_SmartHome/face_recognition/Data/")
            person_num_list = []

            for person in person_list:
                try:
                    # Tách số từ tên tệp/thư mục
                    person_order = person.split('_')[1].split('_')[0]
                    # Kiểm tra nếu phần tách ra có phải là số
                    if person_order.isdigit():
                        person_num_list.append(int(person_order))
                    else:
                        print(f"Warning: '{person}' does not contain a valid number, skipping.")
                except IndexError:
                    print(f"Warning: '{person}' does not match expected format, skipping.")
                except ValueError:
                    print(f"Warning: '{person_order}' is not a valid number, skipping.")

            if person_num_list:
                self.existing_faces_cnt = max(person_num_list)
            else:
                self.existing_faces_cnt = 0  # Hoặc giá trị phù hợp nếu không có giá trị hợp lệ
